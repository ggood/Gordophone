/*
Prototype sketch for a trombone-like MIDI controller running on the
Teensy 2.0 hardware. You will need to run the Arduino 1.0 IDE and get
Teensyduino add-on. I've only tested this on a Mac.
...
- My instrument also has an additional switch, actuated by the right thumb,
  that is currently unused in this sketch. In other incarnations, I've used
  it to enable a slide quantization mode that removes the ability to
  glissando between notes, but instead locks in each of the seven semitones
  the each of the seven slide positions would produce on a traditional
  trombone.
...
// it would send on the MIDI bus. Not sure if this works on the Teensy.
// You may need to select USB type "Serial" from the Teensy menu in the
// Arduino IDE.
...
const int SLIDE_QUANT_TOGGLE_PIN = 6;  // Slide quantization on/off
const int SLIDE_LED_PIN = 13;  // LED that shows slide quantization

const int PANIC_PIN = 7;  // MIDI all notes off momentary switch
...
const int OT_6 = 70;  // Sixth overtone (A flat - not in tune)
...
const int overtone_sw_values[10] = {
    0xff,
    0x00,  // All finger switches open
    0x01,  // Index only
    0x03,  // Index + Middle
    0x07,  // Index + Middle + Ring
    0x0f,  // All finger switches closed
    0x0e,  // Middle + Ring + Pinkie
    0x0c,  // Ring + Pinkie
    0x08,  // Pinkie only
    0xff
};

const int MIDI_VOLUME_CC = 7;  // MIDI volume controller channel
const int MIDI_BREATH_CC = 2;  // MIDI breath controller channel

// Last time we sent continuous data (volume, pb)
long ccSendTime = 0;
// Send CC data no more often than this (ms)
const int MIN_CC_INTERVAL = 10;
// Only send pitch bend if it's this much different than the current value
const int PB_SEND_THRESHOLD = 10;
// Only send volume change if it's this much differnt that the current value
const int VOLUME_SEND_THRESHOLD = 1;
// Raw sensor value required to turn on a note. Will vary by mfr
const int NOTE_ON_VOLUME_THRESHOLD = 56;
// Maximum value from the breath sensor.
const int VOLUME_MAX_VALUE = 500;

// An earlier version of this instrument did not use an actuator on
// the softpot, but instead required the player to touch it with a
// finger. Therefore we needed a way to detect when the player had
// released the softpot, and as it turns out, the analog value read
// is close to the maximum value. In that case we ignore that current
// reading, and continue to sound the same note that was read before
// the player's finger came off the softpot.
const int LPOT_NO_TOUCH_VALUE = 1010;  

// Slide limit constants
...
int slide_quant_mode = 0; // The current slide quantization mode.
 ...
 */
 int getPitchBendFromLinearPot() {
  ...
  if (slideVal > LPOT_NO_TOUCH_VALUE) {
    ...
    constrainedVal = constrainedVal < LPOT_SLIDE_POS_7 ? LPOT_SLIDE_POS_7 :
                     constrainedVal;
    constrainedVal = constrainedVal > LPOT_SLIDE_POS_1 ? LPOT_SLIDE_POS_1 :
                     constrainedVal;
    
   int  pbVal = map(constrainedVal, LPOT_SLIDE_POS_1, LPOT_SLIDE_POS_7,
                    PITCH_BEND_NEUTRAL, MAX_PITCH_BEND_DOWN);
   if (pbVal < 0) pbVal = 0;
...
int getOvertoneFromOvertoneSwitches() {
  ...
  // now select the appropriate overtone - linear search
...
int getVolumeFromBreathSensor() {
  ...
  if (volRawVal < NOTE_ON_VOLUME_THRESHOLD) {
    ...
    return map(constrain(volRawVal, NOTE_ON_VOLUME_THRESHOLD, VOLUME_MAX_VALUE),
              NOTE_ON_VOLUME_THRESHOLD, VOLUME_MAX_VALUE, 0, 127);
  }
}
...
// Main loop. Is repeatedly called by Arduino.
void loop() {
  ...
  if ((-1 != currentNote) && (0 == volume)) {
    ...
    // No note was playing, and we have breath and a valid
    /// overtone, so send a note on.
    // Be sure to send any updated pitch bend first, though,
    //in case the slide moved.
